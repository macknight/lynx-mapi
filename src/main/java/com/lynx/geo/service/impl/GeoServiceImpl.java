...
//	private Logger rgcLog = Logger.getLogger("rgc-log");
    ...
    public GeoPoint geocoding(Address address) {
        ...
        addr = address.getRegion() == null ? addr : String.format("%s%s", addr, address.getRegion());
            ...
            for (Cell cell : cells) {
                ...
                switch (cell.type()) {
                    ...
                    case GSM: {
                        ...
                        locPo = locationDao.getGSMLocation(gsm.getMcc(), gsm.getMnc(),
                ...
                if (locPo != null) {
                    ...
                    if (addr == null) {
                        ...
                        UpdateGeoInfoTask task = new UpdateGeoInfoTask(cell, locPo.getLat(), locPo.getLng());
                        executorService.submit(task);
                    }
                    Location location = new Location(FormatUtil.format(locPo.getLat(), 4),
                            FormatUtil.format(locPo.getLng(), 4),
                            locPo.getAcc(), addr);
    ...
    private Location getBstCellCoord(Map<Cell, Location> cellLocations) {
        ...
        for (int i = 0; i < locations.size(); ++i) {
            ...
            for (int j = 0; j < locations.size(); ++j) {
                dists += GeoUtil.distanceTo(locations.get(i),
                        locations.get(j));
            }
            if (minDist > dists) {
                minDist = dists;
            ...
            try {
                ...
                if (cell instanceof CDMACell) {
                    ...
                    locationDao.updateCDMAAddress(cdma.getMcc(), cdma.getSid(),
                            cdma.getNid(), cdma.getBid(), FormatUtil.address2str(addr));
                } else if (cell instanceof GSMCell) {
                    GSMCell gsm = (GSMCell) cell;
                    locationDao.updateGSMAddress(gsm.getMcc(), gsm.getMnc(),
                            gsm.getLac(), gsm.getCid(), FormatUtil.address2str(addr));
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
