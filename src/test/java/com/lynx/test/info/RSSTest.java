...
public class RSSTest extends BasicTest {
    ...
    public void onlineSinaNewsTest() {
        try {
            List<RSSInfo> infos = SinaRSSUtil.sinaRssNews(1, "http://rss.sina.com.cn/eladies/son.xml");
    ...
    public void test() {
        try {
            infoService.updateSinaRSSInfo(66);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void sinNewsInDBTest() {
        try {
            List<RSSInfo> infos = infoService.getSinaRSSInfo("亲子", 0, new Date().getTime());
            System.out.println(EncryptUtil.format(infos));
        } catch (Exception e) {
            e.printStackTrace();
        }
